  ...
  class AssetReportCreateRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::AssetReportCreateRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::AssetReportCreateRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @access_tokens.length > 99
        invalid_properties.push('invalid value for "access_tokens", number of items must be less than or equal to 99.')
      end
    ...
    def access_tokens=(access_tokens)
      ...
      if access_tokens.length > 99
        fail ArgumentError, 'invalid value for "access_tokens", number of items must be less than or equal to 99.'
      end
